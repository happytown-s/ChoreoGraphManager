...
interface StageProps {
    ...
    onControlPointChange?: (keyframeId: string, dancerId: string, newPos: Position) => void;
    groups?: Group[];
...
const Stage = forwardRef<StageRef, StageProps>(({
    ...
    const drawScene = (
        ...
        if (!isForRecording && usedActivePaths) {
            usedActivePaths.forEach(path => {
                ...
                ctx.lineTo(path.controlPoint.x, path.controlPoint.y);
                ctx.lineTo(path.endPos.x, path.endPos.y);
                ...
                ctx.beginPath();
                ctx.moveTo(path.startPos.x, path.startPos.y);
                ctx.quadraticCurveTo(path.controlPoint.x, path.controlPoint.y, path.endPos.x, path.endPos.y);
                ctx.strokeStyle = 'rgba(56, 189, 248, 0.6)'; // Light blue
                ctx.lineWidth = 2 / currentTransform.k;
                ctx.stroke();

                // Control Point Handle
                const cpSize = 10 / currentTransform.k;
                ctx.fillStyle = '#38bdf8';
                ctx.fillRect(path.controlPoint.x - cpSize / 2, path.controlPoint.y - cpSize / 2, cpSize, cpSize);

                // Handle Border
                ctx.strokeStyle = 'rgba(0,0,0,0.5)';
                ctx.lineWidth = 1 / currentTransform.k;
                ctx.strokeRect(path.controlPoint.x - cpSize / 2, path.controlPoint.y - cpSize / 2, cpSize, cpSize);
            });
        }
    ...
    const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
        ...
        if (pointersRef.current.size === 1) {
            ...
            const isShift = e.shiftKey || e.metaKey || e.ctrlKey || isMultiSelectEnabled;

            // Hit test for Control Points first (Priority over dancers)
            if (activePaths && !isShift && onControlPointChange) {
                for (const path of activePaths) {
                    const cp = path.controlPoint;
                    // Hit area slightly larger than visual
                    const dist = Math.sqrt((wx - cp.x) ** 2 + (wy - cp.y) ** 2);
                    if (dist <= 15) { // 15px radius hit target
                        draggingCPRef.current = {
                            keyframeId: path.keyframeId,
                            dancerId: path.dancerId,
                            anchorWorld: { x: wx, y: wy },
                            startPos: { ...cp }
                        };
                        return; // Stop processing hits
                    }
                }
            }
            ...
            if (hitId) {
                ...
                if (isShift) {
                    ...
                    if (selectedDancerIds.has(hitId)) {
                        // Already selected - mark for potential toggle on pointer up (if no drag)
                        pendingToggleRef.current = hitId;
                    } else {
                        // Not selected - add to selection immediately
    ...
    const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
        ...
        // Control Point Drag
        if (draggingCPRef.current && pointersRef.current.size === 1) {
            const { x: wx, y: wy } = screenToWorld(currentX, currentY);
            const anchor = draggingCPRef.current.anchorWorld;
            let dx = wx - anchor.x;
            let dy = wy - anchor.y;
            ...
            const newPos = clampPosition(
                draggingCPRef.current.startPos.x + dx,
                draggingCPRef.current.startPos.y + dy
            );

            onControlPointChange?.(
                draggingCPRef.current.keyframeId,
                draggingCPRef.current.dancerId,
                newPos
            );
            return;
        }
        ...
        if (multiDragStateRef.current && multiDragStateRef.current.dragging && pointersRef.current.size === 1) {
            ...
            for (const [id, startPos] of Object.entries(multiDragStateRef.current.startPositions)) {
                const pos = startPos as Position;
                const clamped = clampPosition(pos.x + dx, pos.y + dy);
        ...
        if (isDraggingViewRef.current) {
            if (pointersRef.current.size === 2) {
                const values = Array.from(pointersRef.current.values()) as { x: number, y: number }[];
                const p1 = values[0];
                const p2 = values[1];
                const dist = Math.sqrt((p1.x - p2.x) ** 2 + (p1.y - p2.y) ** 2);
    ...
    const handlePointerUp = (e: React.PointerEvent<HTMLCanvasElement>) => {
        ...
        if (pointersRef.current.size === 0) {
            ...
            if (multiDragStateRef.current && multiDragStateRef.current.dragging) {
                const offsets: Record<string, Position> = multiDragStateRef.current.currentOffsets;
                const startPositions: Record<string, Position> = multiDragStateRef.current.startPositions;
                ...
                if (moved) {
                    ...
                    if (pendingToggleRef.current) {
                        const next = new Set(selectedDancerIds);
                        next.delete(pendingToggleRef.current);
                        onSelectDancers(next);
                    }
                }
            }
